...
wb_path <- "D:\\Downloads\\SIOS R 43_Feb_2025.xlsx"
...
target_sheets_service <- all_sheets[grepl("^Table S", all_sheets)]
target_sheets_infrastructure <- all_sheets[grepl("^Table I", all_sheets)]
all_sheets <- c(target_sheets_service,target_sheets_infrastructure)
...
for (sheet in all_sheets) {
    if (sheet == "Table S14") {
        ...
        colnames(df) <- c("Survey_conducted_during", "Reference_quarter",
                          "Response_rate_perc", "Spare_capacity_perc")
        
        # Iterate rows
        for (i in seq_len(nrow(df))) {
            # Convert "Survey conducted during" to a date
            quarter_key <- convert_quarter_date2(df$Reference_quarter[i])
            
            if (is.null(result[[quarter_key]])) {
                result[[quarter_key]] <- list()
            }
            
            # Convert the reference quarter too, if desired
            Survey_conducted_during_converted <- convert_quarter_date2(df$Survey_conducted_during[i])
            
            # Store data under table_name
            result[[quarter_key]][[Category]][[table_name]] <- list(
                "Survey_conducted_during(this_is_not_the_referenced_quarter)" = Survey_conducted_during_converted,
                "Response_rate_(per_cent)" = df$Response_rate_perc[i],
                "Spare_capacity_(per_cent)" = df$Spare_capacity_perc[i]
            )
        }
    }
    else {
        ...
        if (str_detect(string = table_name,pattern = "Table S")) {
            Category <- "Services"
        }
        ...
        df <- read_excel(wb_path, sheet = sheet, skip = 2)
        df <- df[-1,]
        while (dim(df)[2]>9) {
            df <- df[-10]   
        }
        # Rename columns if they are not already correct.
        colnames(df) <- c("Survey_Quarter", "Assessment_Increase", "Assessment_NoChange", 
                          "Assessment_Decrease", "Assessment_NR",
                          "Expectation_Increase", "Expectation_NoChange", 
                          "Expectation_Decrease", "Expectation_NR")
        df <- df |> select(Survey_Quarter,Assessment_NR,Expectation_NR)
        
        df <- df |> filter(Survey_Quarter != "Note: This parameter was introduced in Q2:2017-18") 
        
        
        # Loop over each row of the data frame
        for (i in 1:nrow(df)) {
            quarter <- convert_quarter_date(df$Survey_Quarter[i])
...
# Write JSONL file
jsonl_path <- "D:\\Desktop\\financial_data_pipeline\\data\\raw\\RBI_data\\services_and_infrastructure_survey.jsonl"
