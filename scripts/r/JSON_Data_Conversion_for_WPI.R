...
file_path <- "D:/Downloads/WSS Table No. 09 _ Major Price Indices.xlsx"
json_output_path <- "D:/Downloads/WPI.json"
...
tryCatch({
    ...
    for (sheet in all_sheets) {
        ...
        df <- read_excel(file_path, sheet = sheet,skip = 3,col_types = c("date",rep("numeric",11)))
        
        colnames(df) <- gsub("^\\d+(\\.\\d+)?\\s*", "", colnames(df))
        
        df <- df|> select(Item,c(7:12))
        ...
        df <- df |> mutate(Date = ymd(Item)) |> select(-Item) |> relocate(Date)
