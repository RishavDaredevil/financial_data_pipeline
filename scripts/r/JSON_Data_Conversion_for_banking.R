...
wb_path <- "D:\\Downloads\\BLS Q3-2024-25 - TIME SERIES DATA.xlsx"
...
for (sheet in target_sheets_manufacturing) {
        ...
        table_name <- read_excel(wb_path, sheet = sheet, range = "C1", col_names = FALSE)[[1]] 
        Category <- "Banking"
        ...
        df <- read_excel(wb_path, sheet = sheet, skip = 3)
        df <- df[-1,]
        while (dim(df)[2]>14) {
            df <- df[-15]   
        }
        # Rename columns if they are not already correct.
        colnames(df) <- c("","Survey_Quarter",
                          ...
                          "Assessment_NR",
                          "pi-Substantial Increase",
                          "pi-Moderate Increase",
                          "pi-No change",
                          "pi-Moderate Decrease",
                          "pi-Substantial Decrease",
                          "Expectation_NR")
...
# Write JSONL file
jsonl_path <- "D:\\Desktop\\financial_data_pipeline\\data\\raw\\RBI_data\\banking_survey.jsonl"
