...
class BuildingPerCompletionCertsDataSpider(scrapy.Spider):
    name = "building_per_completion_certs_data"
    allowed_domains = ["briks.gov.in"]
    start_urls = ["https://briks.gov.in/Report/Rept_BuildingPermits.aspx"]
    json_file_path = "D:\\Desktop\\financial_data_pipeline\\data\\raw\\briks_building_india\\building_permits_completion_certs_data.json"
    ongoing_state = None
    ...
    async def parse(self, response):
        ...
        categories_building_permits = [
            "Record No.",
            "State Name",
            "City Name",
            "Quarter",
            "Building Permit Issued Residential",
            "Building Permit Issued Non-Residential",
            "Building Permit Issued Total",
            "Completion Certificate Residential",
            "Completion Certificate Non-Residential",
            "Completion Certificate Total"
        ]
        ...
        for year_used_in_site in years:
            year_to_show = int(year_used_in_site) + 1
            date = f'{year_to_show}-01-01'
            # Select the desired year by its value (e.g., "2023").
            await page.select_option("select#ctl00_ContentPlaceHolder1_ddlyear", value=year_used_in_site)
            await page.wait_for_load_state("networkidle")

            # Click the submit/report button.
            await page.click("a#ctl00_ContentPlaceHolder1_lbtn_Report")
            await page.wait_for_load_state("networkidle")
            ...
            await page.screenshot(
                path='D:\\Desktop\\financial_data_pipeline\\scrapers\\python\\briks_building_india\\images\\screenshot.png',
                full_page=True)
            ...
            for i, row in enumerate(rows):
                ...
                data = row.xpath('./td/table/tbody/tr[1]/td/span/text()').getall()

                record_no_raw = row.xpath('./td[1]/span/text()').get()

                state_text = row.xpath('./td[2]/table/tbody/tr[1]/td/span/text()').get()

                # Check if the extracted state text is non-empty after stripping whitespace
                if state_text and state_text.strip():
                    self.ongoing_state = state_text.strip()
                else:
                    # Use the current self.ongoing_state if state_text is empty
                    state_text = self.ongoing_state
                    # Insert the state_text at the beginning of the data list
                    data.insert(0, state_text)

                if record_no_raw is not None:
                    record_no = record_no_raw.strip()
                    data.insert(0, record_no)
                ...
                # Check if row has more than 9 <td> elements
                if td_count > 9:
                    building_permits_dict = dict(zip(categories, data))
                    results.append(building_permits_dict)
                else:
                    self.logger.warning(f"found less than 9 records row at index {i}")
                    continue

            json_data.update({date: results})

        # await self.check_duplicate_date(date)
    ...
    async def check_duplicate_date(self, date):
        try:
            with open(self.json_file_path, 'r', encoding='utf-8') as file:
                for line in file:
                    data_point = json.loads(line.strip())
                    if date in data_point:  # Check if the date is a key
                        raise CloseSpider(f'Data with date {date} is already present. Closing spider.')
        except FileNotFoundError:
            self.logger.warning(f'File not found: {self.json_file_path}. Proceeding as new file.')
