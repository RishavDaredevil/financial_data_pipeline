...
# Need to schedule this on 20th of every month
...
class RbiMoneySupplySpider(scrapy.Spider):
    ...
    def parse_bulletin(self, response, date):
        ...
        relative_url = re.findall(r'href="([^"]+money-stock-measures[^"]*)"', response.text)[0]
        absolute_url = f'https://website.rbi.org.in{relative_url}'
        yield scrapy.Request(url=absolute_url, callback=self.parse_money_supply, cb_kwargs=dict(date=date))
