...
class OldSiteRbiCpiSpider(scrapy.Spider):
    name = "old_site_rbi_cpi"
    ...
    def parse(self, response):
        ...
        relative_url = response.xpath('//a[contains(text(), "Consumer Price Index")]/@href').get()
        absolute_url = f'https://rbi.org.in/Scripts/{relative_url}'
        self.log(absolute_url)
        yield scrapy.Request(url=absolute_url, callback=self.parse_cpi, cb_kwargs=dict(date=extracted_date))
    ...
    def parse_cpi(self, response, date):
        ...
        for row in rows:
            ...
            if not condition_met and (
                ...
                date_selector = tds[-2:]
                extracted_date_final = self.convert_date_numeric_is_year(
                    date_selector[0].xpath('string()').get().strip(), var_name="extracted_date_final")
                extracted_date_provisional = self.convert_date_numeric_is_year(
                    date_selector[1].xpath('string()').get().strip(), var_name="extracted_date_provisional")
                break
            ...
            if date not in existing_data:
                ...
                for type_key, new_content in data_types.items():
                    ...
                    if type_key not in existing_data[date]:
                        ...
                        both_exist_Already += 1

        if both_exist_Already == 2:
            raise CloseSpider(
                f'Data for both final date and provisional date is already present with full data according to info above. Closing spider.')
