...
class OldSiteRbiNeerReerSpider(scrapy.Spider):
    name = "old_site_rbi_neer_reer"
    ...
    def parse(self, response):
        ...
        relative_url = response.xpath('//a[contains(text(), "Effective Exchange Rate")]/@href').get()
        absolute_url = f'https://rbi.org.in/Scripts/{relative_url}'
        self.log(absolute_url)
        yield scrapy.Request(url=absolute_url, callback=self.parse_neer_reer, cb_kwargs=dict(date=extracted_date))
    ...
    def parse_neer_reer(self, response, date):
        ...
        self.year_of_data = year_extracted - 1 if month_extracted in [1] else year_extracted
        ...
        for row in rows:
            ...
            if not condition_met and (
                ...
                date_selector = tds[-1]
                extracted_date_final = self.convert_date_month_year_to_be_given_seperately(
                    date_selector.xpath('string()').get().strip(),
