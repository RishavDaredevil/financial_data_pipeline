...
class RbiIipSpider(scrapy.Spider):
    name = "rbi_iip"
    ...
    def parse_bulletin(self, response, date):
        ...
        relative_url = re.findall(r'href="([^"]+index-of-industrial-production[^"]*)"', response.text)[0]
        absolute_url = f'https://website.rbi.org.in{relative_url}'
        yield scrapy.Request(url=absolute_url, callback=self.parse_iip, cb_kwargs=dict(date=date))
