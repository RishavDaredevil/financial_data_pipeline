...
class OldSiteRbiWpiSpider(scrapy.Spider):
    name = "old_site_rbi_wpi"
    ...
    def parse(self, response):
        ...
        relative_url = response.xpath('//a[contains(text(), "Wholesale Price Index")]/@href').get()
        absolute_url = f'https://rbi.org.in/Scripts/{relative_url}'
        self.log(absolute_url)
        yield scrapy.Request(url=absolute_url, callback=self.parse_wpi, cb_kwargs=dict(date=extracted_date))
