...
class CurrentMacroDataTradingEcoSpider(scrapy.Spider):
    ...
    json_file_path_for_current_macro_data_trading_eco = r"D:\Desktop\financial_data_pipeline\data\raw\trading_economics"
    Symbol = None
    token_value_spider = None
...
#                     yield {'TELastUpdate': telastupdate_value}
        ...
        if found_TEChartsToken and found_TESymbol:
            ...
            self.token_value_spider = token_value
            try:
                url = f'https://d3ii0wo49og5mi.cloudfront.net/economics/{tesymbol_value}?&span=10y&v={telastupdate_value}&key={token_value}'
                headers = {
                ...
                script_tags_fr_market_symbol = response.xpath('//script[contains(text(), "symbol =")]/text()').getall()

                for script in script_tags_fr_market_symbol:
                    # Use a regex to extract the market_symbol value
                    match = re.search(r"symbol\s*=\s*'([^']+)'", script)
                    if match:
                        self.Symbol = match.group(1).strip()
                        if self.Symbol:
                            self.logger.info("Extracted market_symbol: %s", self.Symbol)
                            yield {'Symbol': self.Symbol}
                            found_market_symbol = True
                            break  # Stop after finding the first valid non-empty value.
                else:
                    self.logger.error("No valid market_symbol value found.")
                ...
                yield scrapy.Request(
                    ...
                    dont_filter=True,
                    cb_kwargs={"name": name, "key": key_value}
                )
            except Exception as e:
                self.logger.error("Request failed; possibly due to different URL structure for %s: %s", name, e)

    def parse_decode_parse(self, response, name, key):

        condition = response.request.meta.get("markets")
        ...
        if condition:
            for item in series_data:
                ...
                formatted_date_str = item[0]
                value = item[1]

                print("1111111111111111",formatted_date_str,value)
    ...
    def errback_handler(self, failure):
        ...
        self.logger.error("Primary request failed for %s", request.url)

        # Log the failure details.
        self.logger.error(repr(failure))

        # Check if the failure is an HTTP error and inspect the response status code.
        if failure.check(HttpError):
            response = failure.value.response
            self.logger.error("HTTP Error code: %s", response.status)

            # If you get a 500 Internal Server Error, try a new URL structure.
            if response.status == 500:
                alternative_url = f'https://d3ii0wo49og5mi.cloudfront.net/markets/{self.Symbol}?interval=1month&span=25y&ohlc=0&key={self.token_value_spider}'
                self.logger.info("Attempting alternative URL: %s", alternative_url)
                try:
                    yield scrapy.Request(
                        alternative_url,
                        headers=request.headers,  # Reuse or modify headers as necessary.
                        callback=self.parse_decode_parse,
                        errback=self.alternative_errback,
                        dont_filter=True,
                        meta={"markets": True},
                        cb_kwargs=request.cb_kwargs
                    )
                except Exception as e:
                    self.logger.error("Alternative URL request failed for %s: %s", request.cb_kwargs["name"], e)
        else:
            # For any other type of error, log the failure.
            self.logger.error("Non-HTTP error encountered: %s", failure)

    def alternative_errback(self, failure):
        request = failure.request
        self.logger.error("Alternative request failed for %s", request.url)
        self.logger.error("Failure details: %s", repr(failure))
